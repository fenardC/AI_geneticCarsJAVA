	...
	public Car(int typeSensor, double ratio) {
		...
		if (typeSensor == ScreenGeneticCar.TYPE_SENSOR_POINT) {
			...
			for (int i = 0 ; i < ScreenGeneticCar.sensorLineNumber ; i++) {
				...
				sensor.setCar(this);
				sensor.setAngle(i / (double) (ScreenGeneticCar.sensorLineNumber - 1) * ScreenGeneticCar.sensorLineAngleCover - ScreenGeneticCar.sensorLineAngleCover / 2);
	...
	public void tick(double millis) {
		...
		for (CarSensorPoint sensor : listRoadCollision) {
			...
			if (sensor.getLastIndexTriangle() > -1) {
				if (sensor.getLastIndexTriangle() > bestIndexSpline && sensor.getLastIndexTriangle() <= bestIndexSpline + maxIndexTriangleDiff) {
					bestIndexSpline = sensor.getLastIndexTriangle();
				} else if (sensor.getLastIndexTriangle() <= maxIndexTriangleDiff && track.getListTriangle().size() <= bestIndexSpline + maxIndexTriangleDiff) {
					bestIndexSpline = sensor.getLastIndexTriangle();
					lap++;
				}
			}
